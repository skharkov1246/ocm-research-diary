/* ====================================================================
   ...
   femoco.html. Verifiedy contract: every NUMBER comes from a real computation in
...
const ENTRIES = [
  /* ---------------------------------------------------------------- */
  {
    ...
    simple: {
      ru: `<p><strong>Азот</strong> нужен всему живому, но в воздухе он связан в инертную молекулу N₂ с тройной связью N≡N — её очень трудно разорвать. Промышленный процесс <em>Габера–Боша</em> делает это при ~400–500 °C и ~150–300 атм и тратит на это порядка 1–2% всей энергии человечества.</p>
           ...
           <p><strong>Наш план — верифицируемый и снизу вверх:</strong> сперва посчитать то, что ещё решается точно (саму молекулу N₂), убедиться, что наш «калькулятор» не врёт, показать, где именно ломается классика, а затем шаг за шагом подниматься к FeMoco.</p>`,
      en: `<p><strong>Nitrogen</strong> is essential to life, but in air it is locked in the inert N₂ molecule with a triple bond N≡N that is very hard to break. The industrial <em>Haber–Bosch</em> process does it at ~400–500 °C and ~150–300 atm, consuming on the order of 1–2% of humanity's total energy.</p>
           ...
           <p><strong>Our plan — verified and bottom-up:</strong> first compute what is still exactly solvable (the N₂ molecule itself), prove our "calculator" doesn't lie, show exactly where classical methods break, and then climb step by step toward FeMoco.</p>`
    },
    tech: {
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    title: { ru: "Движок достоверности: кубитный гамильтониан = CASCI до 9-го знака",
             en: "An verifiedy engine: the qubit Hamiltonian = CASCI to 9 digits" },
    simple: {
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    title: { ru: "Верифицированный калькулятор: разрыв N≡N и срыв классики-одиночки",
             en: "An verified calculator: breaking N≡N and the single-reference collapse" },
    simple: {
      ru: `<p>Прежде чем замахиваться на FeMoco, мы проверяем «калькулятор» на том, что ещё можно решить <strong>точно</strong> — на самой молекуле N₂. Растягиваем связь N≡N шаг за шагом и сравниваем методы (базис cc-pVDZ).</p>
           ...
           <p>Вывод верифицирован и важен: именно <em>этот</em> срыв одно-детерминантных методов на разрыве связи — и есть причина, по которой полный FeMoco неподъёмен для обычной классики. Мы воспроизвели его на пальцах.</p>`,
      en: `<p>Before reaching for FeMoco we test the "calculator" on something still solvable <strong>exactly</strong> — the N₂ molecule itself. We stretch the N≡N bond step by step and compare methods (cc-pVDZ basis).</p>
           ...
           <p>The verified, important point: this very collapse of single-determinant methods at bond breaking is <em>why</em> the full FeMoco is out of reach for ordinary classical methods. We reproduced it from first principles.</p>`
    },
    tech: {
      ru: `<p>Сканирование N₂ по cc-pVDZ, активное пространство для VQE — CAS(6e,6o)=12 кубитов (валентные σ/σ*/π/π* 2p). RHF/CCSD/CCSD(T) — PySCF; CASSCF(6,6) посеян орбиталями AVAS(«N 2p»).</p>
           <ul>
             ...
             <li><strong>CASSCF(6,6):</strong> гладкая кривая, равновесие R≈1.10 Å, «глубина» до R=2.6 Å ≈ <strong>196 ккал/моль</strong> (CAS(6,6) без динамической корреляции занижает абсолютную D<sub>e</sub> — фиксируем явно).</li>
           </ul>
      ...
      en: `<p>N₂ scan in cc-pVDZ; VQE active space CAS(6e,6o)=12 qubits (valence 2p σ/σ*/π/π*). RHF/CCSD/CCSD(T) via PySCF; CASSCF(6,6) seeded with AVAS("N 2p") orbitals.</p>
           <ul>
             ...
             <li><strong>CASSCF(6,6):</strong> smooth curve, equilibrium R≈1.10 Å, "depth" to R=2.6 Å ≈ <strong>196 kcal/mol</strong> (CAS(6,6) without dynamic correlation underestimates the absolute D<sub>e</sub> — noted explicitly).</li>
           </ul>
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    simple: {
      ru: `<p>Теперь измеряем, как быстро задача «раздувается» с размером активного пространства, и проверяем, поспевает ли квантовый алгоритм (VQE) за точным ответом.</p>
           ...
           <p>Важная методическая поправка: в витринах часто пишут «~10¹⁶». Наш прямой подсчёт даёт ≈4×10³⁰ детерминантов (это полная размерность). Каким бы ни был способ счёта — вывод один: точный классический расчёт здесь невозможен, и поэтому FeMoco — задача для будущих квантовых компьютеров.</p>`,
      en: `<p>Now we measure how fast the problem "blows up" with active-space size, and check whether the quantum algorithm (VQE) keeps up with the exact answer.</p>
           ...
           <p>An important verified correction: showcases often quote "~10¹⁶". Our direct count gives ≈4×10³⁰ determinants (the full dimension). Whatever the counting convention, the conclusion is the same: an exact classical calculation is impossible here, which is why FeMoco is a problem for future quantum computers.</p>`
    },
    tech: {
      ru: `<p>На растянутой N₂ (R=1.5 Å, сильная статическая корреляция) считаем CAS(n,n) растущего размера: точный CASCI всегда, ADAPT-VQE до 10 кубитов (бюджет 120 с/точку). Кубитный гамильтониан сверяется с CASCI перед каждым VQE.</p>
           <ul>
             <li><strong>VQE vs CASCI (реально измерено):</strong> CAS(2,2)/4q → 0.63 ккал/моль (2 оператора); CAS(4,4)/8q → 4.43 ккал/моль (19 операторов); CAS(4,5)/10q → 4.37 ккал/моль (23 оператора). Рост ошибки — следствие упёртости в бюджет ADAPT и силы корреляции, а не «подгонки». Достоверно.</li>
      ...
      en: `<p>On stretched N₂ (R=1.5 Å, strong static correlation) we compute CAS(n,n) of growing size: exact CASCI always, ADAPT-VQE up to 10 qubits (120 s/point budget). The qubit Hamiltonian is checked against CASCI before each VQE.</p>
           <ul>
             <li><strong>VQE vs CASCI (really measured):</strong> CAS(2,2)/4q → 0.63 kcal/mol (2 operators); CAS(4,4)/8q → 4.43 kcal/mol (19 operators); CAS(4,5)/10q → 4.37 kcal/mol (23 operators). The growing error reflects ADAPT hitting its budget and the correlation strength, not "tuning". Verified.</li>
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    simple: {
      ru: `<p>Важный методический контрапункт ко всему дневнику. Сама реакция «азот + водород → аммиак» классике <strong>вполне по силам</strong>.</p>
      ...
      en: `<p>An important, verified counterpoint to the whole diary. The overall reaction "nitrogen + hydrogen → ammonia" is <strong>well within classical reach</strong>.</p>
    ...
    tech: {
      ru: `<p>CCSD(T) одноточечно на фиксированных экспериментальных геометриях (H₂ 0.7414 Å; N₂ 1.0977 Å; NH₃ r=1.012 Å, ∠HNH=106.7°). Электронная энергия реакции ΔE = 2E(NH₃) − E(N₂) − 3E(H₂):</p>
           <ul>
             ...
             <li><strong>cc-pVDZ:</strong> CCSD(T) −24.7 ккал/моль — заметно недосходится по базису (~13 ккал/моль неполнота); наглядная иллюстрация важности базиса.</li>
           </ul>
      ...
      en: `<p>CCSD(T) single points at fixed experimental geometries (H₂ 0.7414 Å; N₂ 1.0977 Å; NH₃ r=1.012 Å, ∠HNH=106.7°). Electronic reaction energy ΔE = 2E(NH₃) − E(N₂) − 3E(H₂):</p>
           <ul>
             ...
             <li><strong>cc-pVDZ:</strong> CCSD(T) −24.7 kcal/mol — markedly basis-underconverged (~13 kcal/mol incompleteness); a clear illustration of basis sensitivity.</li>
           </ul>
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    simple: {
      ru: `<p>До сих пор мы работали с самой молекулой N₂ — чтобы отладить инструменты и показать, <em>почему</em> тройная связь трудна. Следующий обоснованный шаг к FeMoco — посадить N₂ на <strong>железо</strong>.</p>
           ...
           <p>Полный FeMoco (10³⁰ — классически невозможно) мы изображать не будем. Идём к нему достоверно, по одной посильной модели за раз, и записываем, что работает, а что нет.</p>`,
      en: `<p>So far we worked with the N₂ molecule itself — to debug the tools and show <em>why</em> the triple bond is hard. The next verified step toward FeMoco is to put N₂ on <strong>iron</strong>.</p>
           ...
           <p>We will not pretend to do the full FeMoco (10³⁰ — classically impossible). We climb toward it explicitly, one tractable model at a time, logging what works and what doesn't.</p>`
    },
    tech: {
      ru: `<p><strong>План следующего этапа</strong> (это план, не результаты — числа заполнит реальный прогон):</p>
           <ul>
             ...
             <li><strong>Железо:</strong> AWS lightning.qubit (где важны RAM/ядра). Ограничения достоверно: квота ~16 vCPU (один c7i.4xlarge зараз); statevector-VQE реалистичен до ~24–28 кубитов, дальше — приближения (фрагментация/embedding).</li>
      ...
      en: `<p><strong>Next-stage plan</strong> (a plan, not results — numbers will be filled by a real run):</p>
           <ul>
             ...
             <li><strong>Hardware:</strong> AWS lightning.qubit (where RAM/cores matter). Constraints stated explicitly: ~16 vCPU quota (one c7i.4xlarge at a time); statevector VQE realistic to ~24–28 qubits, beyond which approximations (fragmentation/embedding) are needed.</li>
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    title: { ru: "N₂ на железе: первая модель самого катализа (и верифицированный сюрприз)",
             en: "N₂ on iron: the first model of catalysis itself (and an verified surprise)" },
    simple: {
      ru: `<p>После калибровки на голом атоме Fe (Этап 8) сажаем N₂ на железо — первая модель самого́ катализа. Оптимизировали геометрию и получили верифицированный, поучительный результат: <strong>одинокий атом железа — плохой активатор</strong>.</p>
           <ul>
      ...
      en: `<p>After calibrating on the bare Fe atom (Stage 8), we put N₂ on iron — the first model of catalysis itself. We optimized the geometry and got an verified, instructive result: <strong>a lone iron atom is a poor activator</strong>.</p>
           <ul>
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    simple: {
      ru: `<p>FeMoco собран из железо-серных блоков. Самый простой — ромб <strong>[2Fe–2S]</strong> ферредоксинов. Его ключевая физика: два железа связывают свои спины <strong>антиферромагнитно</strong> через мостиковые серы — и это <em>принципиально</em> не описывается одной «картинкой» (мультиреференс в чистом виде).</p>
           <p>Мы посчитали это достоверно (трюк broken-symmetry в DFT — он у нас <strong>сошёлся</strong>): антиферромагнитное состояние оказалось ниже ферромагнитного на <strong>11.7 ккал/моль</strong>, что даёт обменную связь <strong>J = −161 см⁻¹</strong> — ровно в экспериментальном диапазоне для таких кластеров. Это и есть та мультиреференсная физика, что делает FeMoco трудным.</p>
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    simple: {
      ru: `<p>На Этапе 11 мы получили J ≈ −161 см⁻¹ (антиферромагнетизм) на B3LYP. Но вся соль железо-серной химии в том, что <strong>ответ DFT зависит от выбранного функционала</strong>. Мы оптимизировали геометрию и пересчитали J <em>четырьмя</em> функционалами DFT.</p>
           ...
           <p>Разброс — <strong>885 см⁻¹, со сменой знака</strong>. То есть DFT не может надёжно сказать даже <strong>знак</strong> магнитной связи [2Fe–2S]. Это и есть предельно наглядное «здесь DFT нельзя верить» — и ровно поэтому нужен верифицированный мультиреференсный (квантовый) метод.</p>`,
      en: `<p>In Stage 11 we got J ≈ −161 cm⁻¹ (antiferromagnetic) with B3LYP. But the whole crux of iron–sulfur chemistry is that <strong>the DFT answer depends on the chosen functional</strong>. We optimized the geometry and recomputed J with <em>four</em> DFT functionals.</p>
           ...
           <p>A spread of <strong>885 cm⁻¹, with a sign flip</strong>. So DFT cannot reliably tell you even the <strong>sign</strong> of the [2Fe–2S] magnetic coupling. This is the clearest possible "you cannot trust DFT here" — and exactly why an verified multireference (quantum) method is needed.</p>`
    },
    tech: {
      ru: `<p>Геометрия [Fe₂S₂(SH)₄]²⁻ <strong>оптимизирована</strong> в высокоспиновом состоянии (B3LYP/def2-SVP, geomeTRIC, сошлась за ~870 с). При этой геометрии — обменная связь по Ямагучи через broken-symmetry, четыре функционала по «лестнице»:</p>
           <ul>
             ...
             <li><strong>TPSSh (мета-гибрид): +706 см⁻¹</strong> — качественно <em>неверно</em> (ферромагнетик); его BS-состояние (⟨S²⟩=3.78) лежит <em>выше</em> HS. Достоверно: это либо патология broken-symmetry для данного функционала, либо настоящий провал — и то, и другое иллюстрирует хрупкость.</li>
           </ul>
      ...
      en: `<p>The [Fe₂S₂(SH)₄]²⁻ geometry was <strong>optimized</strong> at the high-spin state (B3LYP/def2-SVP, geomeTRIC, converged in ~870 s). At that geometry — Yamaguchi exchange coupling via broken symmetry, four functionals across the "ladder":</p>
           <ul>
             ...
             <li><strong>TPSSh (meta-hybrid): +706 cm⁻¹</strong> — qualitatively <em>wrong</em> (ferromagnetic); its BS state (⟨S²⟩=3.78) lies <em>above</em> HS. Verifiedly: either a broken-symmetry pathology for this functional, or a genuine failure — both illustrate the fragility.</li>
           </ul>
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    tech: {
      ru: `<p>[Fe₄S₄(SH)₄]²⁻ кубан (идеализированный куб, минимальная модель, геометрия <em>не</em> оптимизирована), B3LYP/def2-SVP, UKS: 16 атомов, 288 базисных функций, 238 электронов, 18 неспаренных (HS S=9).</p>
           ...
           <p><strong>Лестница кубитов (всё посчитано в дневнике):</strong> N₂ 12 → Fe–N₂ 24 (стена симулятора, Этап 10: VQE не сошёлся за 69 мин) → [2Fe–2S] 56 → [4Fe–4S] 88 → FeMoco CAS(54,54) 108. [4Fe–4S] на 88 кубитах уже почти у FeMoco/108 — далеко за пределами того, где классическая симуляция VQE реальна. Итог дуги: классика-одиночка ломается на физике связи; классическая симуляция VQE ломается на масштабе катализатора (24 кубита); обходной путь DFT теряет даже знак связи. Всё это — количественный, верифицированный аргумент за переход к отказоустойчивому квантовому железу. Оговорки: минимальная модель, геометрия не оптимизирована, размер пространства — оценка.</p>`,
  ...
  },
  {
    ...
    simple: {
      ru: `<p>Лестница довела нас до вершины — <strong>CAS(54,54)</strong> FeMoco, 108 кубитов. Теперь верифицированный вопрос: почему нельзя просто взять очень большой суперкомпьютер и посчитать это «в лоб» (Full CI)? Мы посчитали цену.</p>
      ...
      en: `<p>The ladder brought us to the summit — FeMoco's <strong>CAS(54,54)</strong>, 108 qubits. Now the verified question: why can't we just take a very large supercomputer and do this head-on (Full CI)? We computed the price.</p>
    ...
    tech: {
      ru: `<p>FeMoco CAS(54e,54o) — активное пространство из Reiher et al., PNAS 114, 7555 (2017), «витрина фронтира» квантовой химии.</p>
           ...
           <p>Что посчитано нами и что процитировано — разделено достоверно: <strong>комбинаторика и память — наш точный расчёт</strong>, <strong>T-гейты/время отказоустойчивого алгоритма — оценка Reiher 2017</strong>. Это замыкает аргумент всего дневника: Этапы 1–4 — одиночная классика ломается на физике связи; Этап 10 — классическая симуляция VQE упирается в стену на 24 кубитах; Этапы 12–13 — обходной путь broken-symmetry DFT теряет знак и величину магнитной связи; Этап 14 — а «верифицированный» Full CI для FeMoco требует памяти в 10⁸ раз больше всех данных мира. Вывод один: для каталитического ядра нитрогеназы нужен отказоустойчивый квантовый компьютер.</p>`,
    ...
    figures: [
      { src: "assets/femoco/femoco_estimate.png",
        caption: { ru: "Цена «достоверного» Full CI для FeMoco: один вектор состояния = 6×10³¹ байт, в 600 млн раз больше всех цифровых данных мира. Память посчитана точно; отказоустойчивая оценка ~108 логических кубитов — по Reiher 2017 (цитата).",
                   en: "The price of an 'verified' Full CI for FeMoco: one state vector = 6×10³¹ bytes, 600 million times all the world's digital data. Memory computed exactly; the fault-tolerant estimate ~108 logical qubits is from Reiher 2017 (cited)." } }
    ]
  },
  {
    id: "fe4s4-jscan",
    ...
    simple: {
      ru: `<p>На Этапе 12 мы увидели, как DFT для маленького ромба [2Fe–2S] не может договориться даже о знаке магнитной связи (один функционал из четырёх дал противоположный ответ). Возник верифицированный вопрос: может, это артефакт крошечной модели или неоптимизированной геометрии? Проверяем на большом, FeMoco-релевантном кубане [4Fe–4S] — теперь с <strong>оптимизированной геометрией</strong>.</p>
    ...
    simple: {
      ru: `<p>За пятнадцать этапов этот дневник прошёл путь от молекулы N₂ до полного активного центра FeMoco — и на каждом шаге проверял <strong>расчётом, а не словами</strong> одно утверждение: классические методы не справляются с этой химией, и поэтому нужен квантовый компьютер. Соберём всё вместе.</p>
           ...
           <ol>
             ...
             <li><strong>«Лобовой» точный расчёт невозможен в принципе.</strong> Полный FeMoco — 108 кубитов; один вектор состояния Full CI = 6×10³¹ байт, в 600 млн раз больше всех цифровых данных мира. А квантовому компьютеру нужно лишь ~108 логических кубитов (Этап 14).</li>
           </ol>
      ...
      en: `<p>Across fifteen stages this diary travelled from the N₂ molecule to FeMoco's full active site — and at every step it tested, <strong>by computation rather than assertion</strong>, a single claim: classical methods cannot handle this chemistry, which is why a quantum computer is needed. Here it is in one place.</p>
           ...
           <ol>
             ...
             <li><strong>An "verified" exact calculation is fundamentally impossible.</strong> Full FeMoco is 108 qubits; one Full-CI state vector = 6×10³¹ bytes, 600 million times all the world's digital data. A quantum computer needs only ~108 logical qubits (Stage 14).</li>
           </ol>
  ...
  },
  {
    ...
    simple: {
      ru: `<p>На Этапе 15 broken-symmetry DFT не смог назвать даже <em>знак</em> магнитной связи в [4Fe–4S]. Причина достоверно проверяется здесь: любое DFT/broken-symmetry решение — это <strong>один</strong> детерминант, а основное состояние этого кластера по-настоящему многоконфигурационно. Впервые смотрим на него мультиреференсным методом (CASSCF) — и заодно собираем то, ради чего всё затевается: кубитный гамильтониан.</p>
  ...
  },
  {
    ...
    simple: {
      ru: `<p>Хотели сделать следующий шаг строгости после Этапа 17: досчитать <em>сошедшийся</em> CASSCF и добавить динамическую корреляцию (NEVPT2). Получили результат информативнее и интереснее одной цифры — и он сам по себе аргумент «зачем квант».</p>
           ...
           <p>И индикатор мультиреференсности n_u оказался неустойчив: <strong>2.05</strong> (CASCI) → <strong>2.01</strong> (CASSCF, 3 итерации) → <strong>3.99</strong> (Этап 17, ~50 итераций) — ни одно значение не сошлось. Сам факт мультиреференсности (n_u≫0) робастен, но точное число классически дёшево не зафиксировать. Поэтому n_u=3.99 из Этапа 17 достоверно читаем как <em>неконвергентную</em> оценку (мы это пометили и там).</p>
      ...
      en: `<p>We meant to take the next step in rigor after Stage 17: get a <em>converged</em> CASSCF and add dynamic correlation (NEVPT2). What we got is more verified and more interesting than a single number — and is itself an argument for "why quantum".</p>
    ...
    tech: {
      ru: `<p>[Fe₄S₄(SH)₄]²⁻, def2-SVP, SCF загружен из chkfile Этапа 17 (E_HS=−8230.607 Ha, сошёлся). Фронтир CAS(6e,6o), сплит (3,3) минимального |Sz|.</p>
           ...
           <p><strong>Оговорки (достоверно).</strong> Тот же 6-орбитальный фронтир-субпространство, что на Этапе 17 — демонстрационное, а не полное магнитное пространство Fe-3d (40 кубитов, стена, которую мы не диагонализуем). Идеализированный кубан, def2-SVP, NEVPT2 одностостояние. Стена здесь — не «невозможно в принципе», а «сошедшаяся мультиреференсная опора для реального кластера дорога и хрупка»: это и есть измеренный аргумент за квантовое железо. Всё посчитано в этой сессии; скрипт и results-JSON — <code>calc/femoco_4fe4s_nevpt2.py</code>.</p>`,
