  ...
  },
  {
    ...
    simple: {
      ru: `<p><strong>Этап 17</strong> нашел, что Cr вытягивает селективность в плюс (+0.73), а Fe в минус (−7.19), и самое важное — что DFT выбирает неправильный допант (лучший по NEVPT2 — худший по DFT). Вывод был: надо расширить окно поиска, смотреть на соседей. Этап 17b — вычислительное расширение: берём ТОТ ЖЕ embedded Na/W-каркас с тремя новыми активными металлами из начала 3d-ряда — <strong>Ti (d² d⁰-диамагнитный) и V (d³ d¹-парамагнитный)</strong>.</p>
           ...
           <p><strong>Честно:</strong> это скрин на грубой сетке (5 точек). Абсолютное число +4.18 — нижняя оценка (может быть ещё выше при полной релаксации каркаса). Но <em>ранжирование</em> — очень твёрдое: Ti на вершине, DFT его не видит, это нашей методе можно проверить при любой сетке. Всех пяти металлов считали в их физических основных состояниях (спины разные), поэтому сравнение честное, но «в разных условиях». Каркас заморожен (быстро, физично, но окружение не релаксирует). Дальше надо добить плотной сеткой, может быть и полной геометрией.</p>`,
      en: `<p><strong>Stage 17</strong> found that Cr pulled selectivity into the positive (+0.73) while Fe pulled it negative (−7.19), and most crucially — that DFT picks the wrong dopant. The takeaway was: widen the search window, look at neighbours. Stage 17b is a computational expansion: we use the SAME embedded Na/W frame with three new active metals from the early 3d series — <strong>Ti (d² singlet) and V (d³ doublet)</strong>.</p>
           ...
           <p><strong>Honestly:</strong> this is a screen on a coarse grid (5 points). The absolute number +4.18 is a lower bound (possibly higher under full frame relaxation). But the <em>ranking</em> is very solid: Ti at the apex, DFT cannot see it — this our method can verify at any grid. We computed all five metals at their physical ground states (spins differ), so the comparison is honest but "at different conditions." The frame is frozen (fast, physical, but environment does not relax). Next we need a dense grid push, perhaps even full geometry relaxation.</p>`
    },
    tech: {
      ru: `<p><strong>Постановка:</strong> протокол Этапов 16–17 (embedded Mn=O/Cr/Fe, замороженный Na/W-каркас, UKS-лестница спинов, фикс-AVAS [M 3d, O 2p, 2×H 1s]) продолжен на <strong>расширенный допант-набор из начала 3d-ряда</strong>: Ti и V. Каркас и геометрический протокол — ровно как в Этапе 16. Спины определены UKS лестницей: <strong>Ti 2S=0</strong> (синглет, диамагнитный, CAS(4,5)); <strong>V 2S=1</strong> (дублет, CAS(5,6)). Парировано на одном инстансе (eu-central-1, r7i.12xlarge) параллельно <code>OCM_METAL=Ti,V</code>, leak-proof оркестрация <code>routes/ocm_aws_stage17b.py</code>, пайплайн <code>ocm_mnw_selectivity.py</code>.</p>
           <p><strong>Полный 5-металльный вулкан ΔΔE‡ (ккал/моль) на NEVPT2:</strong> Ti <strong>+4.18</strong> (2S=0); V <strong>+0.85</strong> (2S=1); Cr +0.73 (2S=4); Mn −0.49 (2S=5); Fe −7.19 (2S=6). Квантовые сдвиги (NEVPT2−DFT): Ti <strong>+8.61</strong>; V −0.16; Cr +4.27; Mn +3.30; Fe −5.50. Барьеры NEVPT2 (ккал): Ti CH₄ 8.42 / C₂H₆ 4.24; V 13.29 / 12.44. <strong>Инверсия ранжирования НЕВПТ2 vs DFT:</strong> DFT порядок V(+1.01) > Fe(−1.69) > Cr(−3.54) > Mn(−3.79) > <strong>Ti(−4.43)</strong>; НЕВПТ2 переворачивает: <strong>Ti(+4.18)</strong> > V(+0.85) ≈ Cr(+0.73) > Mn(−0.49) > Fe(−7.19). Лучший по DFT и худший по НЕВПТ2 не совпадают, но наихудший по DFT — лучший по НЕВПТ2 и наоборот (Fe). Все TS интерьерные, CASSCF сошёлся.</p>
           ...
           <p><strong>Error-bar полно:</strong> грубая 5-точечная 2D-пин-сетка (r(C–H), r(O–H), не вариируем углы, не полная оптимизация); step-cap на embedded-релаксе (замороженная Na/W-оболочка); def2-SVP; без ZPE/термики. Ti C₂H₆ барьер 4.24 ккал мал — нужна вери́фикация на более плотной сетке (может быть артефакт положения точки на краю грубого пространства). CASSCF-уровень для всех металлов дан для полноты, но НЕВПТ2-числа — главные (CASSCF без динамкорреляции дает артефакты, особенно для Fe с большим CAS). Спин-состояния разные ([M 3d] конфигурация + оксил-радикал на O), поэтому кросс-сравнение Ti—Fe смешивает смену спина; вывод формулируем как «каждый в физическом основном состоянии». Числа — из <code>routes/ocm_mnw_emb{_ti,_v}_final.json</code>.</p>`,
      en: `<p><strong>The task:</strong> the Stage-16–17 protocol (embedded Mn=O/Cr/Fe, frozen Na/W frame, UKS spin ladder, fixed-composition AVAS [M 3d, O 2p, 2×H 1s]) extended to an <strong>expanded dopant set from the early 3d series</strong>: Ti and V. Frame and geometry protocol — exactly as in Stage 16. Spins determined by UKS ladder: <strong>Ti 2S=0</strong> (singlet, diamagnetic, CAS(4,5)); <strong>V 2S=1</strong> (doublet, CAS(5,6)). Computed on one instance (eu-central-1, r7i.12xlarge) in parallel via <code>OCM_METAL=Ti,V</code>, leak-proof orchestration <code>routes/ocm_aws_stage17b.py</code>, pipeline <code>ocm_mnw_selectivity.py</code>.</p>
           <p><strong>Complete 5-metal volcano ΔΔE‡ (kcal/mol) at NEVPT2:</strong> Ti <strong>+4.18</strong> (2S=0); V <strong>+0.85</strong> (2S=1); Cr +0.73 (2S=4); Mn −0.49 (2S=5); Fe −7.19 (2S=6). Quantum shifts (NEVPT2−DFT): Ti <strong>+8.61</strong>; V −0.16; Cr +4.27; Mn +3.30; Fe −5.50. NEVPT2 barriers (kcal): Ti CH₄ 8.42 / C₂H₆ 4.24; V 13.29 / 12.44. <strong>NEVPT2 vs DFT ranking inversion:</strong> DFT order V(+1.01) > Fe(−1.69) > Cr(−3.54) > Mn(−3.79) > <strong>Ti(−4.43)</strong>; NEVPT2 flips: <strong>Ti(+4.18)</strong> > V(+0.85) ≈ Cr(+0.73) > Mn(−0.49) > Fe(−7.19). Best by DFT and worst by NEVPT2 do not coincide, but DFT's absolute worst is NEVPT2's best and vice versa (Fe). All TS interior, CASSCF converged.</p>
           ...
           <p><strong>Error-bar in full:</strong> coarse 5-point 2D-pin grid (r(C–H), r(O–H), angles fixed, not full geometry); step-cap on embedded relaxation (frozen Na/W shell); def2-SVP; no ZPE/thermal. Ti's C₂H₆ barrier 4.24 kcal is small — needs verification on a denser grid (may be a coarse-space edge artifact). CASSCF level reported for completeness across all metals, but NEVPT2 numbers are primary (CASSCF without dynamic correlation gives artifacts, especially for Fe with large CAS). Spin states differ ([M 3d] configuration + oxyl radical on O), so cross-comparison Ti—Fe mixes a spin change; we frame as "each at its physical ground state." Numbers from <code>routes/ocm_mnw_emb{_ti,_v}_final.json</code>.</p>`
    },
    figures: [
      { src: "assets/ocm_mnw_dopant_volcano_5metal.png",
        caption: { ru: "5-металльный вулкан анти-BEP (embedded Na/W): полная 3d-серия Mn/Cr/Fe/Ti/V в одном каркасе. Слева — ΔΔE‡ по уровням теории (DFT/CASSCF/NEVPT2) для пяти металлов; рейтинг кардинально инвертирован на переходе DFT→NEVPT2. Справа — квантовые сдвиги стрелками: Ti +8.61 (исключительно велик), остальные меньше. Топология: ранние (Ti/V) и поздние (Cr) положительны, Mn нейтрален, Fe резко отрицателен. DFT видит V>Fe>Cr>Mn>Ti; НЕВПТ2 говорит Ti>V≈Cr>Mn>Fe — полная смена. Лучший допант по DFT (V, едва положительный) наихудший по другим критериям; лучший по НЕВПТ2 (Ti) DFT вообще не видит. Реальный прогон, кластер-стенд-ин, грубая сетка.",
                   en: "5-metal anti-BEP volcano (embedded Na/W): full 3d series Mn/Cr/Fe/Ti/V in one frame. Left — ΔΔE‡ by theory level (DFT/CASSCF/NEVPT2) for five metals; ranking inverted dramatically on the DFT→NEVPT2 transition. Right — quantum shifts as arrows: Ti +8.61 (exceptionally large), others smaller. Topology: early (Ti/V) and late (Cr) positive, Mn neutral, Fe sharply negative. DFT sees V>Fe>Cr>Mn>Ti; NEVPT2 says Ti>V≈Cr>Mn>Fe — complete flip. Best dopant by DFT (V, barely positive) is worst by other measures; best by NEVPT2 (Ti) DFT cannot see at all. Real run, cluster stand-in, coarse grid." } }
    ]
  }
];
