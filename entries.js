...
const ENTRIES = [
  ...
  },
  {
    ...
    simple: {
      ru: `<p>В Этапе 15 мы измерили на голом Mn=O-центре: точный расчёт говорит, что катализатор «бережёт» этан сильнее, чем думает обычный DFT (дескриптор ΔΔE‡ уходил в +11.6 — хорошо для селективности). Но голый фрагмент в газе — не реальный катализатор. Этап 16 — тот же замер, но на <strong>кластере с реальным окружением</strong> (первая оболочка Na/W вокруг Mn=O), тяжёлый счёт на AWS.</p>
           <p><strong>Результат честный и двойственный.</strong> Направление квантовой поправки <em>выжило</em>: корреляция снова двигает селективность в «хорошую» сторону, на +3.3 ккал/моль (DFT −3.8 → NEVPT2 −0.5). Тот же знак сдвига, что и на голой модели, — значит принцип «точный расчёт видит то, чего не видит DFT» подтверждён и в окружении. <strong>Но величина резко упала:</strong> в реальном окружении дескриптор почти ноль (−0.5), а не большой плюс, как было на голом фрагменте (+11.6). Окружение почти съедает запас.</p>
           <p><strong>Что это значит по-человечески.</strong> Сам по себе голый Mn=O-центр в настоящем катализаторе — примерно <em>нейтрален</em>: он не бережёт этан заметно сильнее метана. Квантовая «помощь» реальна, но мала. Чтобы получить сильно-селективный центр (тот, что пробивает 28%-стену OCM), одного анти-BEP-запаса голого сайта не хватает — центр нужно «докручивать» (допант или управляющее напряжение). Хорошая новость: сам рычаг (корреляция повышает селективность) работает стабильно и на голой, и на реальной модели — значит его есть куда усиливать инженерно.</p>
           <p><strong>Честно про кухню:</strong> сетка грубая (5 точек), у этана на DFT вершина на краю сетки (его DFT-барьер — нижняя оценка), а «провалы» после перевала в сыром профиле — артефакт грубой сетки, не физика; барьер мы читаем на серединной точке перевала. Окружение заморожено (быстро и физично, но без его релаксации). Это скрин-замер, не финальная проектная цифра.</p>`,
      en: `<p>In Stage 15 we measured, on a bare Mn=O centre, that the accurate calculation says the catalyst "spares" ethane more than plain DFT thinks (the ΔΔE‡ descriptor went to +11.6 — good for selectivity). But a bare gas-phase fragment is not the real catalyst. Stage 16 is the same measurement on an <strong>environment-embedded cluster</strong> (the first Na/W shell around Mn=O), heavy AWS compute.</p>
           ...
           <p><strong>Kitchen honesty:</strong> the grid is coarse (5 points), ethane's DFT maximum sits at the grid edge (its DFT barrier is a lower bound), and the post-saddle "dives" in the raw profile are a coarse-grid artifact, not physics; we read the barrier at the interior saddle point. The environment is frozen (fast and physical, but without its relaxation). This is a screening measurement, not a final design number.</p>`
    },
    tech: {
      ru: `<p><strong>Постановка:</strong> протокол Этапа 15 (<code>ΔΔE‡ = барьер(C₂H₆) − барьер(CH₄)</code>, единое CAS(9e,10o)) перенесён на <strong>embedded-модель</strong> — Mn=O в замороженной первой оболочке Na/W (frozen-frame: реагирующий центр релаксируется, оболочка заморожена — физично и в 4–6× быстрее «плавающей» геометрии). Спин-основное 2S=5 (секстет; лестница 2S=1/3/5/7 → 5 в основном). AVAS фикс-состав [Mn 3d, O 2p, 2×H 1s] ⇒ CAS(9e,10o) на всех структурах. Пайплайн <code>routes/ocm_mnw_selectivity.py</code> в режиме <code>OCM_MODEL=embedded</code>, стадии geom→polish→profile→refine→merge2, идемпотентно/резюмируемо; leak-proof AWS-оркестрация (<code>routes/ocm_aws_stage16.py</code> + супервизор).</p>
           ...
           <p><strong>Error-bar честно:</strong> грубая 5-точечная 2D-пин-сетка + step-cap на embedded-релаксе; C₂H₆ на DFT — <code>ts_interior=false</code> (вершина на краю ⇒ DFT-барьер C₂H₆ = нижняя оценка); сырые пост-TS NEVPT2-точки (−32…−66 ккал) — артефакт продуктовой стороны грубой сетки, барьер берётся на интерьерном TS (index 2). Frozen Na/W-оболочка (без релаксации окружения); def2-SVP; без ZPE/термики; ⟨S²⟩ UKS до ~8.9 (идеал 8.75), коррелир. уровни на ROHF. Числа — из <code>routes/ocm_mnw_emb_final.json</code>.</p>`,
      en: `<p><strong>The task:</strong> the Stage-15 protocol (<code>ΔΔE‡ = barrier(C₂H₆) − barrier(CH₄)</code>, single CAS(9e,10o)) moved to an <strong>embedded model</strong> — Mn=O in a frozen first Na/W shell (frozen-frame: the reacting centre relaxes, the shell is frozen — physical and 4–6× faster than a floppy geometry). Ground spin 2S=5 (sextet; ladder 2S=1/3/5/7 → 5 lowest). Fixed-composition AVAS [Mn 3d, O 2p, 2×H 1s] ⇒ CAS(9e,10o) on every structure. Pipeline <code>routes/ocm_mnw_selectivity.py</code> in <code>OCM_MODEL=embedded</code>, stages geom→polish→profile→refine→merge2, idempotent/resumable; leak-proof AWS orchestration (<code>routes/ocm_aws_stage16.py</code> + supervisor).</p>
           ...
           <p><strong>Honest error-bar:</strong> coarse 5-point 2D-pin grid + a step-cap on the embedded relaxation; C₂H₆ at DFT has <code>ts_interior=false</code> (maximum at the edge ⇒ the DFT C₂H₆ barrier is a lower bound); the raw post-TS NEVPT2 points (−32…−66 kcal) are a product-side coarse-grid artifact — the barrier is read at the interior TS (index 2). Frozen Na/W shell (no environment relaxation); def2-SVP; no ZPE/thermal; UKS ⟨S²⟩ up to ~8.9 (ideal 8.75), correlated levels on ROHF. Numbers from <code>routes/ocm_mnw_emb_final.json</code>.</p>`
    },
    figures: [
  ...
  },
  {
    ...
    simple: {
      ru: `<p>Этап 17b оставил интригу с жирной оговоркой: Ti выглядел чемпионом (+4.18), но его перевал не был пойман внутри сетки — число могло быть миражом, как ранние завышенные цифры у Mn. Этап 18 — <strong>проверка собственного результата на прочность</strong>: та же система, но сетка вдоль пути реакции расширена с 5 до 9 точек (добавлен «продуктовый хвост»), чтобы перевал либо честно поймался, либо честно исчез. Считались все четыре допанта заново: Ti, V, Cr, Fe.</p>
           <p><strong>Итог — образцовый урок честности.</strong> У <strong>Cr и Fe</strong> расширенная сетка воспроизвела пятиточечные числа <em>до сотых</em>: Cr <strong>+0.73</strong>, Fe <strong>−7.19</strong> — надёжные замеры остались надёжными. А вот <strong>Ti-«сенсация» умерла</strong>: с честно пойманным перевалом его барьер для метана оказался 35 ккал (вдвое выше, чем у Cr — слабый центр), а дескриптор сполз с +4.18 к нулю (−0.09). <strong>V выбыл совсем</strong>: у него перевала нет даже в расширенном окне — реакция монотонно в гору (~59 ккал), центр просто неактивен.</p>
           <p><strong>Что осталось стоять.</strong> Главная история — инверсия выбора DFT против точного расчёта — <em>выжила</em>: DFT по-прежнему назвал бы лучшим Fe (наихудший по точному расчёту), а точный расчёт выбирает Cr (+0.73, единственный селективный). Cr теперь подтверждён двумя независимыми сетками. И принцип дороже одного металла: <strong>наш пайплайн сам убил свою красивую сенсацию до того, как она попала бы в патент</strong> — сетка-стресс-тест отличает надёжные числа от миражей. Это и есть цена честности, и она окупается.</p>
  ...
  },
  {
    ...
    tech: {
      ...
      en: `<p><strong>The task:</strong> the Stage-16–18 protocol (embedded Na/W, frozen-frame, 9-point grid, robust readout) on the 4d homologues: <strong>Zr</strong> (2S=0, CAS(4,5)) and <strong>Nb</strong> (2S=1, CAS(5,6)) — spins from scratch ladders, matching the Ti/V parities. Pipeline upgrade: per-metal AVAS d-shell label (<code>Zr/Nb 4d</code>), def2-SVP carries ECP-28 (ecp=BASIS was already passed for the W shell), MINAO coverage verified. Run with harvest-resume (instance deadline mid-Zr → git checkpoint → solo relaunch).</p>
           ...
           <p><strong>Honest error-bar:</strong> the Zr/Ti singlets give zigzag surfaces that pass the neighbour mask (alternating swings) — their magnitudes stay screening-grade even after the robust readout; their verdicts are sign-level, not quantitative. Nb's C₂H₆ and V's profiles are edge-bound — no descriptor by construction. Frozen frame; def2-SVP (ECP-28 for 4d); no ZPE/thermal; each metal at its own ground spin. Numbers: <code>ocm_mnw_emb_{zr,nb}_final{,_robust}.json</code>.</p>`
    },
    figures: [
  ...
  },
  {
    ...
    simple: {
      ...
      en: `<p>After Stages 18–19 one legitimate scepticism remained about the flagship (Cr, ΔΔE‡ = +0.73): everything was computed in the modest def2-SVP basis. The standard referee objection — "your plus will vanish in a big basis." Stage 20 answers with the composite trick: geometries frozen (SVP), all energetics recomputed in <strong>triple-zeta def2-TZVP</strong> — ~2.2× more functions, several-fold costlier per point.</p>
           ...
           <p><strong>A bonus we did not expect.</strong> The intermediate CASSCF level in the small basis produced monstrous barriers (60+ kcal) — we honestly called it "the small-CAS disease". The truth is more interesting: in triple-zeta the CASSCF barriers dropped to reasonable values nearly matching NEVPT2 (15.8/16.1 vs 12.9/13.2), and the CASSCF descriptor even flipped positive (+0.33). The "monster" was largely a <em>basis</em> artifact. One more mirage, caught and labelled.</p>
           <p><strong>The honest frame.</strong> Cr's margin is +0.3…+0.7 kcal: at 800 °C that is an ethane-sparing multiplier of only 1.1–1.4×. A single centre does not break the OCM wall — it works only in a stack of levers (looping without gas-phase O₂ + voltage tuning + the dopant). This is logged in the risk register as the key number, and all economics are computed from the honest multiplier, not from a dream.</p>`
    },
    tech: {
