...
# -*- coding: utf-8 -*-
"""
routes/co2_to_fuels_casscf_crossval.py — НЕЗАВИСИМАЯ кросс-валидация CASSCF-барьера.

Параллельно с routes/co2_to_fuels_casscf.py (реабилитация от воркера-соседа: посайтовый
CAS (12e,10o)/(14e,11o), удержанная релакс-ветка SCF) этот скрипт считает тот же барьер
НАМЕРЕННО другим протоколом: единый CAS(12e,10o) на ОБОИХ сайтах (межсайтовое сравнение
без оговорки о разных пространствах) и warm-chain SCF-ветка «как в скане» (у CuAl она
ложится на ВЕРХНЮЮ из двух SCF-веток — чувствительность CASSCF к ветке меряет Check B в
co2_to_fuels_nevpt2.py). Совпадение двух протоколов: Cu₂ 1.660 vs 1.663, CuAl 1.645 vs
1.650 эВ (3–5 мэВ) — кластерный CASSCF-барьер устойчив к деталям протокола.
...
Ограничения: по-прежнему жёсткая модель — замороженный металл-димер, def2-SVP, нет
...
Запуск: python3 -u routes/co2_to_fuels_casscf_crossval.py
"""
...
def run_site(tag, res):
    ...
    # ДВА PBE-барьера оговорка: (а) ветка ЭТОЙ цепочки — та же, на которой стоит
