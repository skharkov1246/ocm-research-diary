...
# -*- coding: utf-8 -*-
"""
routes/andr_aws.py — leak-proof лаунчер: routes/andrussow_chain.py на r7i.4xlarge в us-west-2.
Защита: terminate-on-shutdown + бортовой shutdown + жнец; DONE-маркер отдельно.

Запуск: python3 routes/andr_aws.py [--dryrun]
"""
import os
import subprocess
import sys
...
rm -f {RM_FILES}
for f in andr_hat_scan.json; do aws s3 cp $S3/$f routes/$f || true; done
mkdir -p /root/scratch
export PYSCF_TMPDIR=/root/scratch
export OMP_NUM_THREADS=$(nproc)
sync_up() {{ aws s3 cp routes/ $S3/ --recursive --exclude '*' --include 'andr_*.json' >/dev/null 2>&1 || true; aws s3 cp /root/repo/andrussow.log $S3/andrussow.log >/dev/null 2>&1 || true; }}
( while true; do sleep 120; sync_up; done ) &
for st in {STAGES}; do
  echo "[aws][andrussow] stage $st" >> /root/repo/andrussow.log
  timeout {STAGE_TO}m python3 -u routes/andrussow_chain.py $st >> /root/repo/andrussow.log 2>&1
  sync_up
done
echo "[aws][andrussow] ALL DONE" >> /root/repo/andrussow.log
sync_up
...
def main():
    ...
    if existing:
        sys.exit(f"andrussow instance(s) already exist: {existing}")
            ...
            try:
                ...
                for f in ("andr_hat_scan.json", "andr_hat.json", "andr_assoc.json", "andr_thermo.json", "andr_results.json"):
                    try:
                        ...
                        pass
                print("[ok] andrussow DONE — results pulled", flush=True)
                return
            except ClientError:
                pass
            time.sleep(60)
