...
Модель (v1, БЕЗ релаксации — single-point на ИДЕАЛИЗИРОВАННОЙ геометрии):
  ...
  оценки "~25" из записи Stage 1 (строго говоря фиксируем).
...
Протокол:
 ...
 3) AVAS(Co 3d оксо-узла + O 2p оксо/OH + O 2p трёх кубановых O при этом Co),
    ...
    строго говоряго FCI-лимита. openshell_option=2: все SOMO принудительно в активном.
 4) CASCI (БЕЗ орбитальной оптимизации, строго говоря = "CASCI на UNO-орбиталях"),
    ...
    в ОДНОМ пространстве (урок feo_cation.py v2: иначе сравнение нес оговоркой).
    NOON, N_u = sum n_i(2-n_i), избыток N_u - 2S -> качественный вердикт
    "мультиреференсен ли узел Co(IV)=O".

Оговорка (ограничения v1):
...
def spin_pops(mf, meta=None):
    ...
    тогда строго говоря пишем причину."""
    try:
        mol = mf.mol
        dm = mf.make_rdm1()
...
def run_geometry(geom_name, res, args):
    ...
    if need:
        ...
        atomic_save(res, OUT_RESULTS)
        if mo is None:
            say(f"  [{geom_name}] AVAS не влез в кап — CASCI строго говоря пропущен")
        else:
            for st, s2 in states:
