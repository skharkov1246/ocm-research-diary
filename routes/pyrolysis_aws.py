...
# -*- coding: utf-8 -*-
"""
routes/pyrolysis_aws.py — запуск пиролиз-дескриптора (pyrolysis_descriptor.py) на
одном EC2 r7i.2xlarge (8 vCPU): панель из 8 пар активный-металл/растворитель,
dft+corr каждая, 2 пары параллельно × 4 потока. Leak-proof: terminate-on-shutdown
+ бортовой watchdog + S3-синк. Результаты (pyrolysis_<PAIR>.json +
pyrolysis_descriptor_results.json) — в s3://alpha-o-results-097743207937/alpha-o/pyro/.

Запуск из песочницы: python3 routes/pyrolysis_aws.py [--dryrun]
"""
import os
import subprocess
import sys
...
MAX_MIN = 660
JOB_MIN = MAX_MIN - 20
PAIRS = "Ni-Bi Cu-Bi Ni-In Pt-Bi Ni-Sn Fe-Sn Co-Ga Pd-Bi"
...
def main():
    ...
    print(f"branch={BRANCH} bucket=s3://{BUCKET}/{PREFIX} pairs={PAIRS}", flush=True)
    ...
    ex = ec2.describe_instances(Filters=[
        {"Name": "tag:Name", "Values": ["pyrolysis"]},
        {"Name": "instance-state-name", "Values": ["pending", "running"]}])
    if any(i for r in ex["Reservations"] for i in r["Instances"]):
        sys.exit("pyrolysis instance already running")
