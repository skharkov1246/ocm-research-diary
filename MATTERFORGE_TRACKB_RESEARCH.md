...
> Статус: **подтверждено и сгенерировано.** Решение пользователя (шаг 2): все системы — в ОДНУ
> отдельную вкладку «Обкатанные технологии»; язык — билингва RU/EN; блок «НЕ прошли в B» (Cu/ZnO,
> Cr-Phillips, VPO) — скрыть. Вкладка реализована (см. `validated_track` в
> `assets/matterforge/matterforge_tabs_content.json` + рендер в `matterforge.js`).
> Достоверно (обновлено 2026-07-02): первый расчёт Дорожки B выполнен — FeO⁺ (спин-состояние
> валидировано: секстет = эксп. ⁶Σ⁺; `calc/feo_cation_results.json`); minimal-модель α-O
> [Fe(O)(OH)₂] прошла CASSCF+NEVPT2 (S=2); real-CHA кластер — в работе. Остальные системы —
> по-прежнему «цель валидации», не «подтверждено».

## Метод и верифицированная оговорка по верификации
